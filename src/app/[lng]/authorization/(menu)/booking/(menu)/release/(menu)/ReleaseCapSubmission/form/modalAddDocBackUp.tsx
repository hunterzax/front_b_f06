...
const ModalAddDocument: React.FC<FormExampleProps> = ({
    ...
    // adapt มาใช้เก็บชื่อไฟล์
    const handleSendComment = (url: any) => {
        ...
        setComments((prev: any) => [...(Array.isArray(prev) ? prev : []), newComment]);
        setDataFileArr((prev: any) => [...(Array.isArray(prev) ? prev : []), newComment])
    ...
    const handleDelete = (url: any) => {
        setComments((prev: any) => prev.filter((file: any) => file.url !== url));
        setDataFileArr((prev: any) => prev.filter((file: any) => file.url !== url));
    };
                    ...
                    <div className="flex flex-col items-center gap-2 p-9 w-[700px]">
                        ...
                        <div className='w-full'>
                            ...
                            <div className={`flex w-full ${errors?.file ? 'border-[#FF0000] h-[39px]' : 'border-none h-[40px]'}`}>
                                ...
                                {/* "Upload" button */}
                                <label className={`${fileName === "Maximum File 10 MB" ? 'bg-[#E5E7EB] !text-[#9CA3AF] pointer-events-none' : 'hover:bg-[#28805a]'} w-[167px] ml-2 !h-[40px] font-bold bg-[#17AC6B] text-white py-2 px-5 rounded-lg cursor-pointer  focus:outline-none  flex items-center justify-center text-[14px] `}> {/* ${isReadOnly && "!bg-[#DFE4EA] !text-[#9CA3AF]"} */}
                                    {`Upload`}
                                    <input
                                        type="button"
                                        className="hidden"
