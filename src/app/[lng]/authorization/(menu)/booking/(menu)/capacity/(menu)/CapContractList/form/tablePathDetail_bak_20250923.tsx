...
const TablePathDetail: React.FC<any> = ({ tableData, dataPeriod, setDataPeriod, dataPathDetailSearch, isReset, setIsReset, dataPathDetail, setDataPathDetail, selectedId }) => {
    ...
    const fetchPathDetail = async (data: any) => {
        ...
        // ========================================================================
        const updatedDataXXX = response?.map((periodItem: any) => ({
            ...periodItem,
            data: periodItem.data.map((areaItem: any) => ({
                ...areaItem,
                dataGroupArea: areaItem.dataGroupArea.map((groupAreaItem: any) => ({
                    ...groupAreaItem,
                    path_display: JSON.parse(groupAreaItem.capacity_detail_point)
                }))
            }))
        }));
        ...
        // if(dataPathDetailSearch?.length > 0 && isReset == false){
        //     setDataPathDetail(dataPathDetailSearch);
        // }else{
    ...
    // useEffect(() => {
    //     fetchPathDetail(tableData);
    // }, [tableData, dataPathDetailSearch, isReset == true])

    const calculateFields = (dataGroupArea: any[]) => {
        ...
        dataGroupArea.forEach((group: any) => {
            ...
            let min_released = Math.min(...group.data.map((item?: any) => {
                return (parseFloat(item.release))
            }))
    ...
    const tableDataX = [
        {
            // data: dataPathDetail?.[0]?.data?.map((item: any) => {
            data: dataToUse?.data?.map((item: any) => {
                // Calculate fields based on item.dataGroupArea
                 
                const calculatedFields: any = calculateFields(item.dataGroupArea);
                        ...
                        <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                            <thead className="text-xs text-[#ffffff] bg-[#1473A1] sticky top-0 z-10">
                                <tr className="h-9">
                                    ...
                                    {/* {
                                        dataPathDetail && dataPathDetail?.[0]?.data?.map((item: any, index: number) => (
                                            <th key={index} scope="col" className={`${table_header_style} text-center`}>
                                                {item.area.name}
                                            </th>
                                        ))
                                    } */}
                            ...
                            <tbody>
                                {[
                                    ...
                                    <tr key={rowIndex} className={`${table_row_style}`}>
                                        {/* Static first column for row labels */}

                                        {columnVisibility.scope && (
                                            <td className="px-2 py-1 min-w-[200px] text-[#464255] sticky left-0 bg-[#FFFFFF]">
                                                {rowLabel}
                                            </td>
                                        )}
                                                                            ...
                                                                            <div>
                                                                                ...
                                                                                {item.path_display?.map((pathGroup: any, groupIndex: number) => (
                                                                                    <div key={groupIndex} className="flex flex-wrap items-center gap-2 pt-2 pb-2">
                                                                                        {pathGroup.map((pathItem: any, index: number) => (
