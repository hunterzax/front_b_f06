...
const ModalSelShipper: React.FC<FormExampleProps> = ({
    ...
    const { control, register, handleSubmit, setValue, reset, formState: { errors }, watch, clearErrors, resetField } = useForm<any>({ defaultValues: {}, });
    ...
    useEffect(() => {
        if (userDT?.account_manage?.[0]?.user_type?.id === 3) {
            setdisFlag(true)
            setIsSelected(true);
            setValue('group_id', userDT?.account_manage?.[0]?.group_id)
            setIdShipper(userDT?.account_manage?.[0]?.group_id) // 2026-03-08 เคสพี่แนน hotfix 2.2.15.0 เดิม api ไม่ให้ส่ง shipper_id ไปกรณีเป็น shipper เล่น แต่จะบังคับส่งไปเลย เพราะต้องใช้อยู่ดี
        }
    }, [open])
