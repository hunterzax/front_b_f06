...
import { epBalancingIntradayBalancingReport } from "@/utils/exportFunc";
...
const ClientPage: React.FC<ClientProps> = (props) => {
    ...
    const getPermission = () => {
        try {
            ...
            // const permission = findRoleConfigByMenuName('Intraday Balancing Report', userDT)
            const permission = findRoleConfigByMenuName('Intraday Balancing Management', userDT)
            if (permission) {
                setUserPermission(permission);
    ...
    const handleReset = async () => {
        ...
        setValue('filter_last_hourly_version', true)
    ...
    const normalize = (value: any): string =>
        (typeof value === 'number' ? formatNumberFourDecimal(value) : value ?? "")
            .toString()
            .replace(/,/g, '')        // 👈 เพิ่มตัวนี้
            .replace(/\s+/g, "")
            .toLowerCase()
            .trim();
        ...
        debounceRef.current = setTimeout(() => {
            ...
            const filtered = filter_plan_actual?.map((entry: any) => {
                ...
                const filteredShipperData = entry.shipperData?.map((shipperGroup: any) => { // --> row blue
                    ...
                    // ของเดิมมันไม่กรอง row ขาวคู่กัน ที่ไม่เจอข้อมูลออก
                    ...
                    // ของใหม่กรองออก https://app.clickup.com/t/86eujrgh2
                    const filteredContracts = shipperGroup?.contractData?.map((contract: any) => { // --> row white
                        const planning = contract?.valueContractPlanning || {};
                        const actual = contract?.valueContractActual || {};

                        const find_shipper = dataShipper?.find((x: any) => x?.id_name === planning?.shipper);
                        const find_shipper_actual = dataShipper?.find((x: any) => x?.id_name === actual?.shipper);

                        // --- text match ---
                        const planningTextMatch = [
                            find_shipper?.name,
                            planning.contract,
                            planning.gas_hour,
                            planning.timestamp,
                        ...
                        // --- value match (กัน undefined/null ให้เป็น "") ---
                        const planningValueMatch = keyValues.some((key) => {
                            const v = planning?.[key];
                            if (v === null || v === undefined || v === "") return false;
                            return normalize(formatNumberFourDecimal(v)).includes(queryLower);
                        });

                        const actualValueMatch = keyValues.some((key) => {
                            const v = actual?.[key];
                            if (v === null || v === undefined || v === "") return false;
                            return normalize(formatNumberFourDecimal(v)).includes(queryLower);
                        });

                        const matched = planningTextMatch || actualTextMatch || planningValueMatch || actualValueMatch;
                        if (!matched) return null;

                        // strip ฝั่งที่ไม่ match ให้เป็น {}
                        const nextPlanning = (planningTextMatch || planningValueMatch) ? planning : null;
                        const nextActual = (actualTextMatch || actualValueMatch) ? actual : null;

                        return {
                            ...contract,
                            valueContractPlanning: nextPlanning,
                            valueContractActual: nextActual,
                        };
                    }).filter(Boolean) || [];
    ...
    const fetchData = async () => {
        ...
        setValue('filter_last_hourly_version', true)
        if(userDT?.account_manage?.[0]?.user_type_id !== 3){
            setValue('filter_only_public', true)
        }
        ...
        try {
            ...
            const statusCode = response?.response?.data?.statusCode ?? response?.response?.data?.status ?? response?.status ?? response?.statusCode ?? response?.code ?? response?.response?.status;
    ...
    const initialColumns: any = [
        ...
        { key: 'instructed_flow_mmbtud', label: 'Instructed Flow (MMBTU/D)', visible: false, parent_id: 'summary_pane' },
        ...
        { key: 'acc_imb_inventory_mmbtud', label: 'Acc. IMB. Inventory (MMBTU/D)', visible: true, parent_id: 'summary_pane' },
        { key: 'min_inventory_mmbtud', label: 'Min. Inventory (MMBTU/D)', visible: false, parent_id: 'summary_pane' },
        ...
        { key: 'east_acc_imb_inventory_mmbtud', label: 'East', visible: true, parent_id: 'acc_imb_inventory_mmbtud' },
        { key: 'west_acc_imb_inventory_mmbtud', label: 'West', visible: true, parent_id: 'acc_imb_inventory_mmbtud' },
    ...
    return (
        <div className=" space-y-2">
            ...
            <div className="border-[#DFE4EA] border-[1px] p-4 rounded-xl  flex flex-col sm:flex-row gap-2">
                <aside className="flex flex-wrap sm:flex-row gap-2 w-full">
                    ...
                    <InputSearch
                        ...
                        onChange={(e) => {
                            setSrchGasHourFrom(e.target.value)
                            setSrchGasHourTo('')
                        }}
                    ...
                    <div className="-mb-2">
                        ...
                        <div className="-mb-2">
                            <CheckboxSearch2
                                ...
                                type="single-line"
                                isDisable={userDT?.account_manage?.[0]?.user_type_id != 3 ? false : true} // v2.0.68 Role TSO ตรง Checkbox  Last Hourly Version ปลด Disable ออก https://app.clickup.com/t/86eu77v3k 
                ...
                <div className="text-sm flex flex-wrap items-center justify-between pb-4">
                    ...
                    <div className="flex flex-wrap gap-2 justify-end">
                        ...
                        <BtnGeneral
                            ...
                            textRender={"Export"}
                            generalFunc={() => {
                                epBalancingIntradayBalancingReport({
                                    name: 'Intraday Balance Report',
            ...
            <ColumnVisibilityPopoverBalReport
                ...
                // initialColumns={initialColumns}
                initialColumns={(userDT?.account_manage?.[0]?.user_type_id == 3 || userDT?.account_manage?.[0]?.user_type_id == 4) ? initialColumns?.filter((item: any) => item?.key !== 'publicate') : initialColumns}
