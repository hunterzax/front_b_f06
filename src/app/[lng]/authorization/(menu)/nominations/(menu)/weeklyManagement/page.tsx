...
const ClientPage: React.FC<ClientProps> = (props) => {
    ...
    const f_filter_gas_day: any = searchParams.get("filter_gas_week");
    ...
    const statusFromParam = nominationStatMaster?.data?.find(
        (item: any) =>
            item?.name?.trim().toLowerCase() ===
            status_from_somewhere_else?.trim().toLowerCase()
    );

    const isParamStatusReady =
        !status_from_somewhere_else || useParams;
    ...
    const [srchShipper, setSrchShipper] = useState<any>(f_group_id ? [(f_group_id)] : []);
    const [srchContractCode, setSrchContractCode] = useState<any>(f_contract_code ? [f_contract_code] : []);
    ...
    const [srchStartDate, setSrchStartDate] = useState<Date | null>(f_filter_gas_day ? dayjs(f_filter_gas_day).toDate() : (toDayjs().day() > 4 ? toDayjs().add(1 , 'week') : toDayjs()).startOf('week').toDate());
...
useEffect(() => {
    if (!status_from_somewhere_else) return;
    if (!statusFromParam?.id) return;
    if (useParams) return;

    setSrchStatus([String(statusFromParam.id)]);
    setuseParams(true);
}, [
...
useEffect(() => {
  console.log('srchShipper : ', srchShipper);
}, [srchShipper])



    // const getAboutDT = async (offset?: any, limit?: any, contract_?:any) => {
    ...
    //             const result = ((contract_?.length > 0 ? contract_ : dataContractOriginal) || [])?.find((f: any) => item == f?.contract_code)
    //             return result
    //         })?.filter((f:any) => !!f)
    //         console.log('find ____ : ', find);
    //         return find
    //     }
    //     console.log('*** srchContractCode : ', srchContractCode);
    ...
    const getAboutDT = async (
        ...
        const searchDayjs = srchStartDate
            ? toDayjs(srchStartDate)
            : null;

        const searchDate =
            searchDayjs?.isValid()
                ? searchDayjs.format("YYYY-MM-DD")
                : null;

        let selectdate: string | null = null;

        // วันที่ที่ผู้ใช้เลือกจาก Filter
        if (searchDate) {
            selectdate = searchDate;
        }
        // วันที่ที่ส่งมาจากหน้า Dashboard
        else if (
        ...
        const newOffset = Math.max(Number(offset ?? 1) - 1, 0);
            ...
            return (options || [])
                .map((item: any) => {
                    return (
                        ...
                        (contract: any) =>
                            String(item) === String(contract?.contract_code)
                    );
                })
                .filter(Boolean);
        };

        const contractID = getContractID(srchContractCode)
        ...
        const body = {
            ...
            offset: newOffset,
            limit,
            gas_day: selectdate, // ไม่เลือกและไม่มี URL param จะเป็น null
            shipper_id_arr:
                srchShipper?.length > 0
                    ? srchShipper.map(Number)
                    : [],
            // contract_id_arr:
            //     contractID.length > 0
            //         ? contractID
            ...
            status_id_arr:
                srchStatus?.length > 0
                    ? srchStatus.map(Number)
                    : [],
            search: ""
        };

        const res_api: any = await postService(
            `/master/query-shipper-nomination-file/v2`,
            body
        );

        if (userDT?.account_manage?.[0]?.user_type_id === 3) {
            const filteredData = res_api?.data?.filter(
                (item: any) =>
                    item?.group?.id ===
    ...
    const handleFieldSearch = async () => {
        ...
        if (filter_group_id_from_somewhere_else) {
            console.log('....srchStartDate : ', srchStartDate);
            contract_ = await apiSelectShipper({ gasDay: dayjs(srchStartDate).format("YYYY-MM-DD") })
            console.log('contract_ : ', contract_);
        }

        // const response: any = await getService(url);
        const res_api = await getAboutDT(currentPage, itemsPerPage, contract_)
        ...
        if (!filter_group_id_from_somewhere_else){
            console.log('_____ : ', filter_group_id_from_somewhere_else);
    ...
    const fetchData = async () => {
        ...
        if (!isNomCodeClick) { // ถ้าคลิก nomcode เข้าไปไม่ต้อง fetch ใหม่ ข้อมูลจะได้ไม่หายเวลากลับออกมา
            ...
            try {
                ...
                if (userDT?.account_manage?.[0]?.user_type_id === 3) {
                    const targetGroupId =
                        groupId || userDT?.account_manage?.[0]?.group_id;

                    const filteredShipperData = (response || []).filter(
                        (item: any) => {
                            const isSameShipper = targetGroupId
                                ? Number(targetGroupId) === Number(item?.group?.id)
                                : true;

                            const isSameGasDay = gasDayFormatted
                                ? gasDayFormatted ===
                                toDayjs(item?.gas_day).format("DD/MM/YYYY")
                                : true;

                            return isSameShipper && isSameGasDay;
                        }
                    );

                    setData(filteredShipperData);
                    setFilteredDataTable(filteredShipperData);
                } else {
                    // มี status จาก URL แต่ไม่มี gas week
                    // API กรอง status มาให้แล้ว ใช้ response ได้เลย
                    if (
                        status_from_somewhere_else &&
                        !filter_gas_week_from_somewhere_else &&
                        !srchStartDate
                        ...
                        const selectedDate = filter_gas_week_from_somewhere_else
                            ? toDayjs(filter_gas_week_from_somewhere_else).format(
                                "YYYY-MM-DD"
                            )
                            : toDayjs(srchStartDate).format("YYYY-MM-DD");

                        const filteredByGasWeek = (response || []).filter(
                            (item: any) =>
                                toDayjs(item?.gas_day).format("YYYY-MM-DD") ===
                                selectedDate
                        );

                        setData(filteredByGasWeek);
                        setFilteredDataTable(filteredByGasWeek);
                    } else {
                        // ไม่มี filter ใด ๆ
                        setData(response || []);
                        setFilteredDataTable(response || []);
            ...
            const res_api = await getAboutDT(
                currentPage,
                itemsPerPage
            );
    ...
    const fetchDataWithParams = async () => {
        ...
        setSrchShipper([Number(filter_group_id_from_somewhere_else)])
  ...
  console.log('dataContractOriginal : ', dataContractOriginal);
    ...
    useEffect(() => {
    // ถ้ามี status จาก URL ให้รอตั้ง srchStatus ก่อน
    if (!isParamStatusReady) return;
    ...
    const openSubmissionModal = async (id?: any, data?: any) => {
        ...
        // setMdSubmissionView(true)
        const data_submission: any = await getService(`/master/query-shipper-nomination-file/submission_comment_query_shipper_nomination_file_version/${id}`);
        const map_data = {
            ...filtered,
            data_submission: data_submission || [],
            submission_comment_query_shipper_nomination_file: data_submission?.[0]?.submiss || []
        }
        setVersion(data_submission?.[0]?.id ? `${data_submission?.[0]?.id}` : null)
        setDataSubmission(map_data)
        setMdSubmissionView(true)
    };
   ...
   useEffect(() => {
    ...
    const apiSelectShipper = async ({ gasDay }:any) => {
        let apiUrl = `/master/daily-adjustment/shipper-data?gas_day=${gasDay}`
        ...
        console.log('res_ : ', res_);
          if (userDT?.account_manage?.[0]?.user_type_id == 3) {
                ...
                setDataShipper(res_ || []) // https://app.clickup.com/t/86ewwu518 ที่มา set ยังงี้เพราะ master ที่เอาไว้ทำ option มันได้มาจากข้อมูล ถ้าไม่มี ชื่อ shipper จะไม่ขึ้น
    ...
    // console.log('-> : ', filter_gas_week_from_somewhere_else
    //                                 ? filter_gas_week_from_somewhere_else
    //                                 : srchStartDate
    //                                     ? srchStartDate
                ...
                <div className="border-[#DFE4EA] border-[1px] p-4 rounded-xl flex flex-col sm:flex-row gap-2">
                    <aside className="flex flex-wrap sm:flex-row gap-2 w-full">
                        <DatePickaSearch 
                            ...
                            //         : srchStartDate
                            //             ? srchStartDate
                            //             : (dayjs().day() === 5 || dayjs().day() === 6 ? dayjs().add(1, 'week').day(0) : dayjs().day(0).format("YYYY-MM-DD"))
                            // }
                        ...
                        <InputSearch
                            ...
                            options={filter_contract_code_from_somewhere_else ? dataContractOriginal?.filter((item: any) =>
                                    srchShipper.length > 0
                                        ? srchShipper.includes(String(item.group_id))
                                        : true
                                )
                                    .map((item: any) => ({
                                        // value: item?.id?.toString(),
