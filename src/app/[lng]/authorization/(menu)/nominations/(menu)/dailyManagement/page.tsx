...
const ClientPage: React.FC<ClientProps> = (props) => {
    ...
    const [srchStartDate, setSrchStartDate] = useState<Date | null>(
        f_filter_gas_day
            ? dayjs(f_filter_gas_day).toDate()
            : toDayjs().add(1, 'day').startOf('day').toDate()
    );
    ...
    const getAboutDT = async (
        ...
        if (srchStartDate) {
            const selectedDate = toDayjs(srchStartDate);

            if (selectedDate.isValid()) {
                selectdate = selectedDate.format("YYYY-MM-DD");
            }
        } else if (filter_gas_day_from_somewhere_else) {
            const paramDate = toDayjs(
                filter_gas_day_from_somewhere_else
            );

            if (paramDate.isValid()) {
                selectdate = paramDate.format("YYYY-MM-DD");
            }
        }
        ...
        const contractID = (srchContractCode || [])
        ...
        const statusValues = Array.isArray(srchStatus)
            ? srchStatus
            : srchStatus
                ? [srchStatus]
                : [];

        const shipperValues = Array.isArray(srchShipper)
            ? srchShipper
            : srchShipper
                ? [srchShipper]
                : [];
        ...
        const body = {
            ...
            limit: limit ?? 10,
            gas_day: selectdate,
            shipper_id_arr: shipperValues
                .map(Number)
                .filter(Number.isFinite),
            // contract_id_arr: contractID,
            contract_code_arr: srchContractCode || [],
            status_id_arr: statusValues
                .map(Number)
                .filter(Number.isFinite),
            search: ""
        };
    ...
    const handleFieldSearch = async () => {
        ...
        if (filter_group_id_from_somewhere_else) {
            contract_ = await apiSelectShipper({ gasDay: localDate })
        }
    ...
    //         const response = Array.isArray(res_api?.data)
    //             ? res_api.data
    //             : [];
    ...
    try {
        ...
        const storedFilter =
            localStorage.getItem("h593stkin2xqa9m");

        const filterObj = storedFilter
            ? JSON.parse(storedFilter)
            : null;

        if (IsSearchClick && filterObj?.gas_day) {
            setSrchStartDate(
                toDayjs(filterObj.gas_day).toDate()
            );
        }
        ...
        getService(
            `/master/asset/concept-point-query?type_concept_point_id=3&start_date=${gasDayString}&end_date=${gasDayString}`
        ).then((conceptPointList) => {
            if (Array.isArray(conceptPointList)) {
                setDataConceptPoint(conceptPointList);
            }
        });
    } catch (error) {
    ...
    useEffect(() => {
    // มี status จาก URL ต้องรอให้ Status Master
    // และ srchStatus พร้อมก่อน
                ...
                <div className="border-[#DFE4EA] border-[1px] p-4 rounded-xl flex flex-col sm:flex-row gap-2">
                    <aside className="flex flex-wrap sm:flex-row gap-2 w-full">
                        ...
                        <DatePickaSearch
                            ...
                            dateToFix={srchStartDate}
                            onChange={(e: any) => setSrchStartDate(e ? e : null)}
                            defaultValue={srchStartDate ?? null}
                        />
