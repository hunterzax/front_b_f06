...
const [smartquery, setsmartquery] = useState<string>("");
  ...
  const handleClick = (
    ...
    if (id !== activeButton) {
      setIsLoading(true);
        // filter_menu ? Number(filter_menu) : null
  ...
  const transformMENU = (data: any) => {
    // <---- data = ข้อมูลที่มากับ account
    const disabledMENU: string[] = ["PMIS GRAPHICS", "DASHBOARD & REPORT"];
        ...
        .map((item: any) => {
          ...
          if (item?.menu?.some((m: any) => m?.menus_config_id === 64)) {
            ...
            let endpoints = allMenus.map((m) => {
              const checkPath: any = usePath?.find(
                (d: any) => d?.key == m?.name,
              );
              return {
          ...
          if (item?.name?.toUpperCase() === "CAPACITY MANAGEMENT") {
            return [
              item?.name?.toUpperCase(),
              {
                keys: [...keys, ...newKeys].map((m) => m?.toUpperCase()),
                endpoints: [...endpoints, ...newEnpoint_],
              },
            ];
          } else {
            return [item?.name?.toUpperCase(), { keys, endpoints }];
  ...
  function handleUpdateActiveButtonData({
    ...
    const menuConfig: any = menu_config[menuName as keyof typeof menu_config];
    ...
    if (isFetchData == true) {
      ...
      const menu_ =
        menuName === "CAPACITY MANAGEMENT"
          ? [...menuConfig?.keys, ...newKeys]
          : [...menuConfig?.keys];

      // Filter and map data based on configuration
      menu_.map((key: string) => {
        // item?.name?.toUpperCase() === "CAPACITY MANAGEMENT"
        ...
        if (targetKey) {
          ...
          if (query) {
            if (
              ...
              (value?.remainingTasks &&
                `${value.remainingTasks}`.includes(query))
            ) {
          ...
          const getEndpoint = menuConfig?.endpoints?.find(
            (d: any) =>
              d?.key.trim().toUpperCase() === key.trim().toUpperCase(),
          );
  ...
  const fetchSubMenuData = async (menuName: string, endPoint: any) => {
    ...
    try {
      ...
      return []; // return เมื่อได้ครบทุก response แล้ว
    } catch (error) {
      return [];
    }
  };
