...
import {exportToExcel, findRoleConfigByMenuName, formatDate, generateUserPermission, toDayjs} from '@/utils/generalFormatter'
...
const newKeysBook = ['Capacity Contract List (Saved)', 'Capacity Contract List (Confirmed)', 'Capacity Contract Management (Saved)', 'Capacity Contract Management (Confirmed)']

const newKeysEvent = ['Offspec Gas (Acknowledge)', 'Emergency/Difficult Day (Acknowledge)', 'OFO/IF (Acknowledge)']
...
const ClientPage: React.FC<ClientProps> = (props) => {
  ...
  const handleClick = async (id: number | undefined, res_?: any, subRes_?: any) => {
    if (!id) return

    const currentRequestId = ++menuRequestRef.current

    const currentSubMenu = subRes_?.length > 0 ? subRes_ : subMenu

    const selectedMenu = currentSubMenu?.find((item: any) => item?.id === id)

    if (!selectedMenu) return

    const menuName = selectedMenu?.name?.trim() ?? ''
    const normalizedMenuName = menuName.toUpperCase()
    ...
    try {
      /**
       * Login Tracking ไม่ใช้ Waiting List API
       */
      if (normalizedMenuName === 'LOGIN TRACKING') {
        await fetchData()

        // ป้องกัน response เก่าทับเมนูล่าสุด
        if (currentRequestId !== menuRequestRef.current) return

        return
      }

      /**
       * เรียก API เฉพาะเมนูที่กด
       *
       * Capacity Management
       * Allocation
       * Nominations
       * Event
       */
      const response = await fetchWaitingListByMenu(menuName)

      // ถ้าผู้ใช้กดเมนูอื่นไปแล้ว ไม่ต้องเอา response เก่ามาแสดง
      if (currentRequestId !== menuRequestRef.current) return
      ...
      console.error(`Cannot load menu "${menuName}"`, error)

      setActiveButtonData({})
    } finally {
      if (currentRequestId === menuRequestRef.current) {
        setIsLoading(false)
      }
    }
  }
  ...
  const getPermission = () => {
    if (user_permission) {
      try {
        ...
        const permission = findRoleConfigByMenuName('Dashboard & Report', userDT)
        if (permission) {
  ...
  const fetchData = async () => {
    try {
      ...
      const data_res_user = response?.data.sort((a: {update_date: string}, b: {update_date: string}) => {
        return new Date(b.update_date).getTime() - new Date(a.update_date).getTime()
      })
      console.log('data_res_user : ', data_res_user)
      const newData: any = []
  ...
  const fetchWaitingListByMenu = async (menuName: string) => {
    try {
      const response = await getService(`/master/waiting-list/v2?menuName=${encodeURIComponent(menuName)}`)
  ...
  const detailCard = (key: string) => {
    ...
    const getDetail: any = listDetail?.find((item: any) => item?.key == key)?.desc || null
  ...
  const handleFieldSearch = () => {
    ...
    const result = dataTable?.filter((item: any) => {
      return (srchLginMode || srchLginMode !== '' ? srchLginMode == item?.login_mode?.id : true) && (srchGroup?.length > 0 ? srchGroup.includes(item?.company_name || '') : true) && (srchType?.length > 0 ? srchType.includes(item?.type_account?.name || '') : true)
    })
  ...
  // function handleUpdateActiveButtonData({isFetchData = true, newData, menuName, query, res_, res_main_data}: {isFetchData?: boolean; newData?: any; menuName: string; query?: string; res_?: any; res_main_data?: any}) {
  ...
  //   const menu_ = menuName === 'CAPACITY MANAGEMENT' ? [...menuConfig?.keys, ...newKeysBook] : menuName === 'EVENT' ? [...menuConfig?.keys, ...newKeysEvent] : [...menuConfig?.keys]

  //   menu_.map((key: string) => {
  //     const targetKey = keys.find((k: string) => k.trim().toUpperCase() === key.trim().toUpperCase())
  //     if (targetKey) {
  ...
  //       }
  //     } else {
  //       const getEndpoint = menuConfig?.endpoints?.find((d: any) => d?.key.trim().toUpperCase() === key.trim().toUpperCase())
  ...
  function handleUpdateActiveButtonData({isFetchData = true, newData, menuName, query, res_, res_main_data}: {isFetchData?: boolean; newData?: any; menuName: string; query?: string; res_?: any; res_main_data?: any}) {
    const currentMenuConfig = res_ ?? menu_config

    const menuConfig = currentMenuConfig?.[menuName as keyof typeof currentMenuConfig]
    ...
    const menuKeys = menuName === 'CAPACITY MANAGEMENT' ? [...menuConfig.keys, ...newKeysBook] : menuName === 'EVENT' ? [...menuConfig.keys, ...newKeysEvent] : [...menuConfig.keys]

    menuKeys.forEach((key: string) => {
      const targetKey = keys.find((currentKey: string) => currentKey.trim().toUpperCase() === key.trim().toUpperCase())
      ...
      if (targetKey) {
        ...
        if (query) {
          ...
          if (key.toUpperCase().includes(normalizedQuery) || (value?.remainingTasks !== undefined && String(value.remainingTasks).includes(query))) {
            data[targetKey] = value
          }
        } else {
          data[targetKey] = value
        ...
        const endpoint = menuConfig?.endpoints?.find((item: any) => item?.key?.trim()?.toUpperCase() === key.trim().toUpperCase())
    ...
    const combinedArray = [...dataArray, ...otherObj]
    const resultObj = Object.fromEntries(combinedArray)

    setActiveButtonData(resultObj)

    // Cache ข้อมูลแต่ละเมนูไว้รวมกัน
    setWaitingListData((prev: any) => ({
      ...prev,
      ...resultObj
    }))
  ...
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
          ...
          return (
            <div className="flex justify-start items-center absolute !w-auto" style={{transform: 'translate(-8px, -13px)'}}>
              <div
                ...
                style={{
                  backgroundColor: row?.status ? row?.account_manage[0]?.mode_account.color : '#EFECEC'
                }}
              >
      ...
      },
      {
        ...
        cell: (info) => {
          const row: any = info?.row?.original
          return <div className={`${row?.status ? 'text-[#464255]' : 'text-[#9CA3AF]'} w-[150px] whitespace-normal break-all leading-snug`}>{row?.user_id ?? ''}</div>
        }
      },
      {
      ...
      },
      {
        ...
        cell: (info) => {
          const row: any = info?.row?.original
          return <div className={`${row?.status ? 'text-[#464255]' : 'text-[#9CA3AF]'}`}>{row?.login_logs?.length > 0 ? formatDate(row?.login_logs[0]?.create_date) : ''}</div>
        }
      },
      {
        ...
        cell: (info) => {
          ...
          return <div className={`${row?.status ? 'text-[#464255]' : 'text-[#9CA3AF]'}`}>{duration}</div>
        }
      }
    ],
  ...
  const getInit = async () => {
    ...
    const {res_, subRes_} = transformMENU(menus_config)

    /**
     * ถ้ามี ?menu=... ให้เรียก API เฉพาะเมนูนั้น
     */
    if (filter_menu) {
      await handleClick(Number(filter_menu), res_, subRes_)
    }
  }
  ...
  const transformMENU = (data: any) => {
    ...
    const getMasterMENU = Object.fromEntries(
      originalMenuWaitingList.map((item: any) => {
        ...
        leafMenus = leafMenus.filter((m) => {
          const match = data?.find((d: any) => d?.menus_id === m?.menus_config_id && d?.b_manage === true)
        ...
        if (item?.menu?.some((m: any) => m?.menus_config_id === 64)) {
          const dailyMenus = leafMenus.filter((m) => m?.url?.toLowerCase()?.includes('daily'))
          const weeklyMenus = leafMenus.filter((m) => m?.url?.toLowerCase()?.includes('weekly'))

          const allMenus = [...leafMenus.filter((m) => !m?.url?.toLowerCase()?.includes('daily') && !m?.url?.toLowerCase()?.includes('weekly')), ...dailyMenus, ...weeklyMenus]
    ...
    const getMenuList = [
      ...
      ...originalMenuWaitingList
        .filter((item) => {
          return data.some((d: any) => d?.menus_id === item?.menus_config_id && d?.b_manage === true)
        })
        .map((item) => {
          return {
    ...
    console.log('getMasterMENU : ', getMasterMENU);
    ...
    <div className="h-[calc(100dvh-10%)] overflow-hidden">
      {isLoading && (
          <div className="absolute inset-0 z-[9999] flex items-center justify-center bg-white/60">
            <Spinloading spin={true} rounded={20} />
          </div>
        )}
      <div className="flex h-full gap-2 overflow-hidden">
        <NavMenuCustom toggleNav={toggleNav} setToggleNav={setToggleNav} subMenu={subMenu} isLoading={isLoading} handleClick={handleClick} activeButton={activeButton} />
                ...
                <div>
                  <div className="border-[#DFE4EA] border-[1px] p-4 rounded-xl flex flex-col sm:flex-row gap-2 shadow-md">
                    <aside className="flex flex-wrap sm:flex-row gap-2 w-full">
                      ...
                      <InputSearch
                        ...
                        options={Array.from(new Map((dataTable ?? []).map((item: any) => [item.company_name, item])).values()).map((item: any) => ({
                  ...
                  <div className="h-[calc(100dvh-255px)] overflow-x-auto">
                    <AppTable
                      ...
                      columns={columns}
                      isLoading={!isLoading}
                      exportBtn={<BtnGeneral bgcolor={'#24AB6A'} modeIcon={'export'} textRender={'Export'} generalFunc={() => exportToExcel(dataExport, 'login-tracking', columnVisibility)} can_export={userPermission ? userPermission?.f_export : false} />}
                        ...
                        return (
                          ...
                          >
                            <div className="h-[70%] w-full">
                              <p className="font-bold text-md text-[#58585A] line-clamp-2">{key == 'OF/IF' ? 'OFO/IF' : key}</p>
                              {activeButtonData[key]?.status && <div className="mb-3">{`Status : ${activeButtonData[key]?.status}`}</div>}
                              {detailCard(getPath?.key) && <div className="mb-6 opacity-70 text-sm">{detailCard(getPath?.key)}</div>}
                            </div>
                            <div className="h-[30%] flex flex-column sm:flex-row flex-wrap items-end justify-between">
                              <div className="w-full h-full flex items-center justify-between">
                                ...
                                <div className="flex flex-wrap justify-end items-center font-bold text-5xl text-[#1473A1]">{activeButtonData[key]?.remainingTasks}</div>
                              </div>
                            </div>
                          </div>
                        )
...
const NavMenuCustom = ({toggleNav, setToggleNav, subMenu, isLoading, handleClick, activeButton}: any) => {
  return (
    <nav className={`relative h-[calc(100vh-135px)] border-[#DFE4EA] border-[1px] p-2 rounded-tr-xl ${toggleNav ? 'w-[250px] text-sm' : 'w-[92px] text-[0px]'} duration-200 ease-in-out space-y-2`}>
      <div className={`absolute top-[20px] right-[-9px] border-[#DFE4EA] border-[1px] rounded-md cursor-pointer bg-white `} onClick={() => setToggleNav(!toggleNav)}>
      ...
      <div className="h-[calc(100vh-245px)] flex flex-col overflow-hidden">
        <div className="overflow-y-auto h-full scrollbar-hide">
          <section className={`${!toggleNav ? 'pl-[15px]' : 'pl-[5px]'} relative pt-2 `}>
            <header
              className={`
                    relative
            ...
            >
              <section className="flex items-center gap-2 py-1">
                ...
                {toggleNav ? (
                  <h1 className={`break-words whitespace-normal text-[#757575] text-[12px]`}>
                    <div className={`flex justify-center items-center px-2 h-[26px] text-[#000000] text-[14px]`}>Waiting List</div>
                  </h1>
                ) : (
