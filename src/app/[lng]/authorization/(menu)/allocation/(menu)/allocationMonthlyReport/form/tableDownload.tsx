...
const TableDownload: React.FC<TableProps> = ({ openViewForm, tableData, isLoading, columnVisibility, userPermission, setIsLoading, setModalErrorMsg, setModalErrorOpen }) => {
    ...
    const downloadFile = async (id: any, row?: any) => {
        try {
            setIsLoading?.(false);
            // const file_name = row?.file
            const file_name = `Allocation Monthly Report ${row?.monthText} (Rev.${row?.version?.split("V.")?.[1]})`
            // balance เปลี่ยนโครงสร้างใช้แบบนี้แต่ allo ยังใช้อันบนไปก่อน const file_name = row?.rev ? row?.file : `(Data Old Not Suport) Monthly Report ${row?.monthText} (Rev.${row?.rev || "(Data Old Not Suport)"})`
                            ...
                            {sortedData && sortedData?.map((row: any, index: any) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {columnVisibility?.group && (
                                            // <td className="px-2 py-1 text-[#464255]">{row?.contractCode ? row?.group?.name : ''}</td> // ถ้ามี contractCode ค่อยแสดงชื่อ shipper
                                            <td className="px-2 py-1 text-[#464255]">{row?.group?.name ? row?.group?.name : ''}</td>
                                        )}
